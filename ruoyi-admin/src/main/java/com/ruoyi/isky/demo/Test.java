...
class Test {
    ...
    public static boolean insertSQLite(String sql, List list) throws IOException {
        ...
        String fileName = SQLiteJDBCLoader.class.getResource("/") + "../../db/lxzwcj.db";//数据库文件地址
        try {
    ...
    /**
     *查询  可以仿照insertSQLite(,,)方法写出通用查询，在此不再重复
     *查询结果存入map  因为公司一直用 ibatis总数据持久层操作，故有此习惯；可灵活修改
     */
    public static Map selectSQLite(String sql) {
        ...
        Statement stm = null;
        ResultSet rs = null;
        String fileName = SQLiteJDBCLoader.class.getResource("/") + "../../db/lxzwcj.db";
        try {
            Class.forName("org.sqlite.JDBC");

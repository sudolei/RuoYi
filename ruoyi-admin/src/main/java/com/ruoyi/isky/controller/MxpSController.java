...
@Controller
@RequestMapping({"/mxps"})
public class MxpSController extends BaseController {
    ...
    @GetMapping({"/veditPic"})
    public String veditPic(HttpServletRequest request, String imageId,String suffix, String tpid, ModelMap mmap) {
        HttpSession session = request.getSession();
        String u_s_e_id = (String) session.getAttribute("u_s_e_id");
        if (StringUtils.isEmpty(u_s_e_id)) {
            return vlogin();

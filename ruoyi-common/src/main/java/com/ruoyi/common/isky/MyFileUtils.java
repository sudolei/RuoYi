...
public class MyFileUtils extends org.apache.commons.io.FileUtils {
	...
	public static boolean copyFile(String srcFileName, String descFileName) {
		return MyFileUtils.copyFileCover(srcFileName, descFileName, false);
	}
	...
	public static boolean copyFileCover(String srcFileName, String descFileName, boolean coverlay) {
		...
		if (descFile.exists()) {
			...
			if (coverlay) {
				...
				if (!MyFileUtils.delFile(descFileName)) {
	...
	public static boolean copyDirectory(String srcDirName, String descDirName) {
		return MyFileUtils.copyDirectoryCover(srcDirName, descDirName, false);
	}
	...
	public static boolean copyDirectoryCover(String srcDirName, String descDirName, boolean coverlay) {
		...
		if (descDir.exists()) {
			if (coverlay) {
				...
				if (!MyFileUtils.delFile(descDirNames)) {
		...
		for (int i = 0; i < files.length; i++) {
			// 如果是一个单个文件，则直接复制
			if (files[i].isFile()) {
				flag = MyFileUtils.copyFile(files[i].getAbsolutePath(), descDirName + files[i].getName());
				// 如果拷贝文件失败，则退出循环
				if (!flag) {
					break;
				}
			...
			if (files[i].isDirectory()) {
				flag = MyFileUtils.copyDirectory(files[i].getAbsolutePath(), descDirName + files[i].getName());
				// 如果拷贝目录失败，则退出循环
				if (!flag) {
					break;
				}
	...
	public static boolean delFile(String fileName) {
		...
		if (!file.exists()) {
			...
			if (file.isFile()) {
				...
				return MyFileUtils.deleteDirectory(fileName);
			}
		}
	}
	...
	public static boolean deleteDirectory(String dirName) {
		...
		for (int i = 0; i < files.length; i++) {
			...
			// 删除子目录
				...
				flag = MyFileUtils.deleteDirectory(files[i].getAbsolutePath());
				// 如果删除子目录失败，则退出循环
				if (!flag) {
					break;
				}
	...
	public static void writeToFile(String fileName, String content, boolean append) {
		try {
			MyFileUtils.write(new File(fileName), content, "utf-8", append);
	...
	public static void writeToFile(String fileName, String content, String encoding, boolean append) {
		try {
			MyFileUtils.write(new File(fileName), content, encoding, append);
			logger.debug("文件 " + fileName + " 写入成功!");
		} catch (IOException e) {
			logger.debug("文件 " + fileName + " 写入失败! " + e.getMessage());
		}
	...
	public static void zipFiles(String srcDirName, String fileName, String descFileName) {
		...
		try {
			...
			if ("*".equals(fileName) || "".equals(fileName)) {
				MyFileUtils.zipDirectoryToZipFile(dirPath, fileDir, zouts);
			} else {
				File file = new File(fileDir, fileName);
				if (file.isFile()) {
					MyFileUtils.zipFilesToZipFile(dirPath, file, zouts);
				} else {
					MyFileUtils.zipDirectoryToZipFile(dirPath, file, zouts);
				}
			}
			zouts.close();
	...
	public static void zipToFiles(String srcDirName, List<String> fileNames, String descFileName) {
		// 判断目录是否存在
		if (srcDirName == null) {
			logger.debug("文件压缩失败，目录 " + srcDirName + " 不存在!");
			return;
	...
	public static void zipDirectoryToZipFile(String dirPath, File fileDir, ZipOutputStream zouts) {
		if (fileDir.isDirectory()) {
			...
			for (int i = 0; i < files.length; i++) {
				if (files[i].isFile()) {
					// 如果是文件，则调用文件压缩方法
					MyFileUtils.zipFilesToZipFile(dirPath, files[i], zouts);
				} else {
					// 如果是目录，则递归调用
					MyFileUtils.zipDirectoryToZipFile(dirPath, files[i], zouts);
				}
			}
		}
	}
